            ...
            if (options.conditions) {
                    ...
                    if (!readCursor) {
                        ...
                        if (options.conditions[index.keyPath] instanceof Array) {
                            ...
                            if (options.conditions[index.keyPath][0] > options.conditions[index.keyPath][1]) {
                                ...
                                readCursor = index.openCursor(bounds, "prev");
                            } else {
                                // We want ASC order
                                readCursor = index.openCursor(bounds, "next");
                            }
                ...
                if (options.range) {
                    ...
                    if (options.range[0] > options.range[1]) {
                        readCursor = store.openCursor(bounds, "prev");
                    } else {
                        readCursor = store.openCursor(bounds, "next");
                    }
                } else {
